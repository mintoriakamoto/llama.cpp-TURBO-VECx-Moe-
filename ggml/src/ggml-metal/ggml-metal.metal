...
#define WRITE_ROWS FC_gated_delta_net_write_rows
    ...
    // input state read base, layout [S_v, S_v, H, n_seqs] (s0 only): per-seq
    // stride is H*D. rows mode: s is a 2D cache view with D-wide contiguous
    // rows and seq i23's live state sits at cache row rows[i23].
    // state is stored transposed: M[i20][is] = S[is][i20], so row i20 is contiguous
    const uint state_seq = HAS_ROWS ? (uint)((device const int *) rows)[i23] : (uint) i23;
    const uint state_in_base = (state_seq*args.ne21 + i21)*S_v*S_v + i20*S_v;
